...
class recorder:
    ...
    def move(self):
        ...
        while not rospy.is_shutdown():
            ...
            # xc,yc,tc=est_pos
            xc = recorder.xs  # x position
            yc = recorder.ys  # y position
            tc = recorder.yaw  # angle
            rx = [0.75, 0.75, 1]  # target x positions
            ry = [0, 0.5, 0.5]  # target y positions
            ed = 0  # last error is intially 0
            edi = 0  # integral accumulator is initially 0

            for i in range(len(rx)):  # iterate through target coordinates
                # longitudinal error (forward distance)
                S = (rx[i] - xc) * np.cos(tc) + (ry[i] - yc) * np.sin(tc)

                # lateral error (sideways distance)
                d = (ry[i] - yc) * np.cos(tc) - (rx[i] - xc) * np.sin(tc)

                # move to next point if this point is near/behind the robot
                if S > 0.08:
                    break

            # print(S,d)
            v_cmd, w_cmd, ed, edi = pid_controller(d, ed, edi, kP=5, kI=0.5)
            ...
            print(round(v_cmd, 4), round(w_cmd, 4))
            #########################################################################
